...
//    }
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        //1.取得表格資料並檢查      
        request.setCharacterEncoding("UTF-8");//瀏覽器所送的文字資料編碼
        String id = request.getParameter("id");//請求取得參數(表單資料),form表單裡的input標籤的NAME="id"
        String pwd = request.getParameter("pwd");
        String checkCode = request.getParameter("checkCode");//檢核碼
        ...
        if (checkCode == null || (checkCode = checkCode.trim()).length() == 0) {
            ...
            String rand = (String) session.getAttribute("ImageCheckServlet");//取得屬性(id="值")
